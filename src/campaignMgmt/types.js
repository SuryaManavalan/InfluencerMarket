export const CAMPAIGN_REGISTER = 'campaign_register';
export const CAMPAIGN_UPDATE = 'campaign_update';
export const CAMPAIGN_EDIT = 'campaign_edit';
export const CAMPAIGN_EDIT_SUCCESS = 'campaign_edit_success';
export const CAMPAIGN_CREATE = 'campaign_create';
export const CAMPAIGN_CREATE_FAIL = 'campaign_create_fail';
export const CAMPAIGN_CREATE_INIT = 'campaign_create_init';
export const CAMPAIGN_CREATE_SUCCESS = 'campaign_create_success';
export const CAMPAIGN_CAT_LIST = 'campaign_cat_list';
export const CAMPAIGN_LIST_FAIL = 'campaign_list_fail';
export const CAMPAIGN_LIST_SUCCESS = 'campaign_list_success';
export const NEW_CAMPAIGN_LIST_SUCCESS = 'new_campaign_list_success';
export const REG_CAMPAIGN_LIST_SUCCESS = 'reg_campaign_list_success';
export const MORE_CAMPAIGN_LIST_SUCCESS = 'more_campaign_list_success';
export const CAMPAIGN_SEARCH_SUCCESS = 'campaign_search_success';
export const CAMPAIGN_SEARCH_INIT = 'campaign_search_init';
export const CAMPAIGN_DELETE_SUCCESS = 'campaign_delete_success';

export const MY_CAMPAIGNS = 'my_campaigns';
export const NEW_CAMPAIGNS = 'new_campaigns';
export const OTHER_CAMPAIGNS = 'other_campaigns';
export const TRENDING_CAMPAIGNS = 'trending_campaigns';





